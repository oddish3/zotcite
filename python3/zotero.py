...
class ZoteroEntries:
    ...
    def __init__(self):
        ...
        if os.getenv("ZoteroSQLpath") is None:
            if os.path.isfile(os.path.expanduser("~/Zotero/zotero.sqlite")):
                ...
                self._errmsg(
                    "The file zotero.sqlite was not found. Please, define the value of `zotero_SQL_path` in your zotcite config."
                )
                return None
        else:
            ...
            if os.path.isfile(zpath):
                ...
                if os.path.isdir(zpath):
                    ...
                    if os.path.isfile(zpath):
                        ...
                        self._errmsg(
                            'Please, check if `zotero_SQL_path` in your zotcite config is correct: "'
                            + zpath
                            + '" not found.'
                        )
                        return None
    ...
    def GetNotes(self, key, clean):
        ...
        def item2ref(s):
            ...
            s = re.sub(
                "\001(........)\002", lambda k: key2ref(k.group(), clean), s, flags=re.M
            )
    ...
    def CreateNote(self, zotkey, vault_path=""):
        """Create a styled markdown note for a citation with annotations and notes

        zotkey (string): The Zotero key as it appears in the markdown document.
        vault_path (string): Path to save the note to (if empty, uses current directory)
        """
        for k in self._e:
            if self._e[k]["zotkey"] == zotkey:
                entry = self._e[k]
                ...
                authors = ""
                if "author" in entry:
                    for last, first in entry["author"]:
                ...
                # Get Zotero annotations (highlighted text)
                annotations = self.GetAnnotations(zotkey, 0, clean=True, md=False)
                quotes = []
                comments = []
                ...
                # Get Zotero notes
                notes = self.GetNotes(zotkey, True)
                if notes:
...
---
    """
                # Define header and abstract sections
                header = f"""
# {title} ({year})

## Summary
<!-- Brief 2-3 sentence summary of the paper's main contribution -->
...
"""
                ...
                # Quotes section with annotations
                quotes_section = """
## Quotes
"""
                if quotes:
                ...
                # Notes section with Zotero notes
                notes_section = """
## Notes
"""
                if notes:
...
"""
                ...
                # Determine save path (vault path or current directory)
                save_path = vault_path.strip() if vault_path.strip() else os.getcwd()
                ...
                if not os.path.exists(save_path):
                    try:
                        ...
                        self._errmsg(f"Failed to create directory: {save_path}")
                        return ""

                # Save to the specified path
                filepath = os.path.join(save_path, f"{citekey}.md")
