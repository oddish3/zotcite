local config = {
  ...
  obsidian_vault_path = "", -- Path to store Obsidian notes (empty uses current directory)
  insert_obsidian_link = true, -- Whether to insert wiki-links after creating notes
  log = {},
}
...
local set_path = function()
  ...
  if vim.fn.has("win32") == 1 then
    ...
    if not vim.env.PATH:find(zpath) then vim.env.PATH = zpath .. ";" .. vim.env.PATH end
  else
...
local global_init = function()
  ...
  vim.api.nvim_create_user_command(
    "Zinsert",
    function(tbl) require("zotcite.get").insert_field(tbl.args) end,
    {
      nargs = "+",
      desc = "Zotcite: insert field from citation",
      complete = function(ArgLead, CmdLine, CursorPos)
        ...
        if ArgLead == "" then return common_fields end

        local filtered = {}
        for _, field in ipairs(common_fields) do
          if field:find(ArgLead, 1, true) == 1 then table.insert(filtered, field) end
        end
        return filtered
      end,
    }
  ...
  vim.api.nvim_create_user_command(
    "Zmakenote",
    function(tbl) require("zotcite.get").create_note(tbl.args) end,
    { nargs = "?", desc = "Zotcite: insert note from citation" }
  )
...
M.init = function()
  ...
  if not M.has_buffer(vim.api.nvim_get_current_buf()) then
    ...
    create_map(
            ...
            "<Plug>ZCreateNote",
            "<leader>zn",
            "<Cmd>lua require('zotcite.get').create_note()<CR>",
            "Zotcite: Create note from citation"
    )
